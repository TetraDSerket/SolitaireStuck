...
public class SolitairePanel extends JPanel
{
	...
	public SolitairePanel()
	{
		...
		this.setBackground(new Color(15,35,110));
...
//			g.drawImage(IHomestuck,0,this.getHeight()-(int)(mouseClicks*4),null);
		...
		if(gameFinished)
		{
			int x,y,width,height,aw,ah,initx,inity;
			initx = (this.getWidth()-Icover.getWidth())/2;
			inity = (this.getHeight()-Icover.getHeight())/2;
			x=190; y=280; width=415; height=110; aw=30; ah=30;
			g.drawImage(Icover,initx,inity,null);
			g.setColor(Color.black);
			g.fillRoundRect(initx+x,inity+y,width,height,aw,ah);
			g.setColor(Color.green);
			g.drawRoundRect(initx+x,inity+y,width,height,aw,ah);
			...
			g.drawString("You Won!!!!!!!!" ,initx+x+10, inity+y+60); //800 650
			g.setFont(new Font("ScoreFont", Font.PLAIN, 30));
			g.setColor(Color.orange);
			g.drawString("Your Score is: "+(1104-mouseClicks), initx+x+75, inity+y+95);
		}
	}
